...
class TriageSession:
    ...
    def from_dynamodb_item(cls, item: dict) -> 'TriageSession':
        ...
        try:
            history_data = json.loads(item.get("History", "[]"))
            history = [ConversationMessage(**msg) for msg in history_data]
        except (json.JSONDecodeError, TypeError):
            history = []

        created_at = None
        if created_str := item.get("CreatedAt"):
            try:
        ...
        last_updated = None
        if updated_str := item.get("LastUpdated"):
            try:
                last_updated = datetime.fromisoformat(updated_str)
            except (ValueError, TypeError):
                pass

        # Parse PK and SK to get channel_id and thread_ts
        pk = item.get("PK", "")
        sk = item.get("SK", "")
        channel_id = pk.replace("CHANNEL#", "") if pk.startswith("CHANNEL#") else pk
        thread_ts = sk.replace("THREAD#", "") if sk.startswith("THREAD#") else sk

        return cls(
            channel_id=channel_id,
            thread_ts=thread_ts,
            original_user_id=item.get("OriginalUserID", ""),
            status=item.get("Status", "active"),
            suggested_title=item.get("SuggestedTitle", ""),
